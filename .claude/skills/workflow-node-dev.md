...
- [ ] 单元测试已编写
- [ ] 文档已更新
- [ ] Docker 容器已同步修改（backend + backend_worker）
...
项目使用 Docker Compose 部署，涉及代码的容器：

| 容器 | 说明 | 镜像 |
|------|------|------|
| `bisheng-backend` | API 服务 | dataelement/bisheng-backend |
| `bisheng-backend-worker` | Celery 异步任务 | dataelement/bisheng-backend |

**注意**：修改节点代码后，需要同时更新两个容器。
...
services:
  backend:
    volumes:
      - ../src/backend/bisheng:/app/bisheng:ro
      - ../src/backend/bisheng_langchain:/app/bisheng_langchain:ro
    command: sh -c "uvicorn bisheng.main:app --host 0.0.0.0 --port 7860 --reload --workers 1"
...
docker compose -f docker-compose.yml -f docker-compose.dev.yml -p bisheng up -d
```
...
# 将 image: dataelement/bisheng-backend:v2.4.0-beta1
# 改为 image: bisheng-backend:dev
...
```bash
# 复制单个文件
docker cp /path/to/my_new_node.py bisheng-backend:/app/bisheng/workflow/nodes/my_new_node/
docker cp /path/to/my_new_node.py bisheng-backend-worker:/app/bisheng/workflow/nodes/my_new_node/
...
如果修改了 `pyproject.toml` 添加新依赖，**必须重新构建镜像**：
...
# 进入容器检查文件
docker exec -it bisheng-backend ls -la /app/bisheng/workflow/nodes/
```
...
| 方式 | 适用场景 | 优点 | 缺点 |
|------|----------|------|------|
| 开发模式挂载 | 频繁开发调试 | 修改即时生效 | 需确保依赖兼容 |
| 本地构建镜像 | 正式部署/完整测试 | 环境完整可靠 | 构建较慢 |
| 临时复制文件 | 快速验证少量修改 | 简单快速 | 不适合大改动 |